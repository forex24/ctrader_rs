...
impl ContinueBar {
    ...
    pub fn update(&mut self, event: &ProtoOaSpotEvent) -> (Quote, Option<Candle>) {
        ...
        if !event.trendbar.is_empty() {
            // update OHLC
            debug_assert!(Self::map_api_period_to_minutes(event.trendbar[0].period.unwrap() as u32) == self.period);
        ...
        let quote = Quote {timestamp:event.timestamp.unwrap(), ask:self.last_ask, bid: self.last_bid};
        (quote, result)
    }
}